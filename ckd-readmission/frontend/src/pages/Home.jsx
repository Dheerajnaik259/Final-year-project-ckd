  ...
  const handleSubmit = async (data) => {
    ...
    try {
      ...
      try {
        ...
        const recordId = result.prediction_id || `local_${Date.now()}`;
        const newRecord = {
          id: recordId,
          user_id: user?.id,
        ...
        localStorage.setItem(
          storageKey,
          JSON.stringify([newRecord, ...existing.filter((item) => item.id !== recordId)])
        );
      } catch (err) {
      ...
      <header className="prediction-header-row">
        <div className="header-title-lockup">
          ...
          <p className="prediction-sub-desc">
            Enter clinical and laboratory details to calculate readmission risk, KDIGO staging, and
            KFRE prognosis.
          </p>
        </div>
        ...
        <div className="disclaimer-left">
          ...
          <div className="disclaimer-text-lockup">
            <span className="disclaimer-title">
              Clinical decision support, not a replacement for judgment
            </span>
            <span className="disclaimer-sub">
              This tool provides risk estimates to support clinical decisions and should be used
