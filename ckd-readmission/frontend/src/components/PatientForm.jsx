  ...
  return (
    <div className="entry-workspace-split">
      ...
      <div className="entry-form-card">
        ...
        {error && <div className="error-banner" style={{ margin: "1.25rem 2rem 0 2rem" }}>{error}</div>}
        ...
        <div className="entry-fields-grid">
          {visibleFields.map((field) => {
            ...
            return (
              <div className={`entry-field-card ${fieldErrors[field.key] ? "invalid" : ""}`} key={field.key}>
        ...
        <div className="form-bottom-notice-bar">
          <div className="notice-left-info">
            <InfoCircleIcon />
            <span>
              Optional lab fields left blank will assume standard baseline defaults. <strong>Note:</strong> Prediction accuracy may vary if actual lab values differ.
            </span>
          </div>

          <div className="notice-actions-right">
            {step > 0 && (
              <button className="btn-signin-outline" type="button" onClick={() => setStep((current) => current - 1)}>
                Back
              </button>
            )}
            {step < 2 ? (
      ...
      <aside className="entry-right-sidebar">
        <div className="whats-next-card">
          ...
          <div className="sidebar-tip-box">
            <LightbulbIcon />
            <span>Complete all sections to generate an accurate risk assessment and prognosis.</span>
          </div>
        </div>
      </aside>
    </div>
